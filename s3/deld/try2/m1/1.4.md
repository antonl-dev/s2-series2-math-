...
3.  **Fractional Part (0.75):** `0.75₁₀ = .11₂` (from section 1.1). To fill 4 fractional bits, we pad with zeros: `.1100₂`.
...
*   Fractional part: `0.625₁₀ = .101₂` (0.625 * 2 = 1.25 -> 1; 0.25 * 2 = 0.5 -> 0; 0.5 * 2 = 1.0 -> 1)
*   Combined: `12.625₁₀ = 1100.101₂`

**Step 3: Normalize the Binary Number**
Normalize the number so there is only one non-zero digit to the left of the binary point. This means moving the point and adjusting the exponent.
`1100.101 = 1.100101 × 2³`
The actual exponent is **3**.

**Step 4: Calculate the Biased Exponent (E)**
The formula is `Biased Exponent = Actual Exponent + Bias`. For single-precision, the bias is 127.
`E = 3 + 127 = 130`
Now, convert `130₁₀` to an 8-bit binary number: `130₁₀ = 10000010₂`.

**Step 5: Determine the Mantissa (M)**
The mantissa is the fractional part of the normalized number. The leading `1` is implicit.
Normalized number: `1.100101`
Mantissa (23 bits): `10010100000000000000000` (pad with zeros to fill 23 bits).

**Step 6: Assemble the Final 32-bit Number**
Combine the three parts: Sign | Biased Exponent | Mantissa
