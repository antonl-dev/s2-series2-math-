...
> **Visual Aid:** A number circle is the best way to understand 2's complement.
>
> [Click here for a 4-bit 2's Complement Number Circle](https://www.google.com/search?tbm=isch&q=4-bit+2's+complement+number+circle)
>
> **What to look for:** A circular diagram with numbers 0 through 15. It should show that `0000` to `0111` represent `+0` to `+7`, and that the circle continues seamlessly from `+7` to `-8` (`1000`) up to `-1` (`1111`). This visually demonstrates the single zero and the wrap-around nature of the representation.
...
6.  **Interpret the result:** The result is negative and in 1's complement form. To find its decimal magnitude, we take the 1's complement of the result:
    ...
    `00100011₂ = 32 + 2 + 1 = 35₁₀`. (The handwritten note's calculation seems to have an error, the correct answer for 14-45 should be -31). The procedure shown is what matters.
